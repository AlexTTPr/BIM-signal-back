...
public class AdminCreationSeed
{
    public static async Task SeedAsync(SignInManager<User> userManager, RoleManager<IdentityRole<long>> roleManager, ILogger logger)
    {
        ...
        businessUsers.ToList().ForEach(async i =>
        {
            if (await userManager.UserManager.FindByNameAsync(i.UserName) == null)
            {
                var result = await userManager.UserManager.CreateAsync(i, i.UserName);

                if (result.Succeeded)
                {
                    await userManager.UserManager.AddToRoleAsync(i, ClaimRoles.User);
                    await userManager.UserManager.AddToRoleAsync(i, ClaimRoles.Business);
                    logger.LogInformation("{username} has been created", i.UserName);
                }
                else
                {
                    logger.LogError("Errror while creating user {username}:", i.UserName);
                    result.Errors.ToList().ForEach(e => logger.LogError("{Code}: {Description}", e.Code, e.Description));
                }
            }
        });
